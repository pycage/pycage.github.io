...
var STATIC_BASE = 1024, STACK_BASE = 5283696, STACKTOP = STACK_BASE, STACK_MAX = 40816, DYNAMIC_BASE = 5283696, DYNAMICTOP_PTR = 39888;
...
var __main_thread_futex_wait_address = 40800;
...
function __embind_register_std_string(rawType, name) {
 ...
 registerType(rawType, {
  ...
  "fromWireType": function(value) {
   ...
   var str;
   if (stdStringIsUTF8) {
    var endChar = GROWABLE_HEAP_U8()[value + 4 + length];
    var endCharSwap = 0;
    if (endChar != 0) {
     endCharSwap = endChar;
     GROWABLE_HEAP_U8()[value + 4 + length] = 0;
    }
    var decodeStartPtr = value + 4;
    for (var i = 0; i <= length; ++i) {
     var currentBytePtr = value + 4 + i;
     if (GROWABLE_HEAP_U8()[currentBytePtr] == 0) {
    ...
    if (endCharSwap != 0) {
     GROWABLE_HEAP_U8()[value + 4 + length] = endCharSwap;
    }
   } else {
...
var proxiedFunctionTable = [ null, _atexit, ___sys_munmap, ___sys_read, _emscripten_set_canvas_element_size_main_thread, _fd_close, _fd_read, _fd_seek, _fd_write ];
